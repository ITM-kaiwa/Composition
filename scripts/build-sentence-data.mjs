...
function tokenize(tokenizer, text) {
  const tokens = tokenizer.tokenize(text).map((t) => t.surface_form);
  const tiles = [];
  for (const tok of tokens) {
    if (ATTACH_TO_PREVIOUS.has(tok) && tiles.length > 0) {
      tiles[tiles.length - 1] += tok;
    } else {
      tiles.push(tok);
    }
  }
...
for (const lessonNum of lessonsFound) {
  ...
  for (const cat of ["bunkei", "reibun", "renshuA"]) {
    ...
    for (const spacedText of bucket[cat]) {
      ...
      const tiles = tokenize(tokenizer, plain).filter(Boolean);
